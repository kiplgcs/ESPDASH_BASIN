...
constexpr unsigned long kModbusMinPollMs = 200; // Background polling floor for unchanged RS485 data.
constexpr unsigned long kModbusDefaultPollMs = 1000; // Changed relay commands are immediate; background readback is slow.
...
inline unsigned long modbusBackgroundPollIntervalMs() {
  unsigned long configured = Rs485PollIntervalMs > 0 ? static_cast<unsigned long>(Rs485PollIntervalMs) : kModbusDefaultPollMs;
  if (configured < kModbusMinPollMs) configured = kModbusMinPollMs;
  if (configured > 5000UL) configured = 5000UL;
  return configured;
}

inline bool queueRelayWriteIfNeeded(uint8_t relay, bool desired, uint8_t &writesThisPass) {
  if (relay >= 16) return false;
  if (!Rs485Enabled) return true;
...
inline void pollModbusInputsAndRelays() {
  if (!Rs485Enabled) return;
  static unsigned long lastInputPoll = 0;
  static unsigned long lastRelayPoll = 0;
  unsigned long now = millis();
  const unsigned long pollInterval = modbusBackgroundPollIntervalMs();
  const bool forcePoll = Rs485ForcePoll;
  if (forcePoll) {
    AktualReadInput = false;
    AktualReadRelay = false;
  }

  if (!AktualReadInput && (forcePoll || now - lastInputPoll >= pollInterval) && modbusQueueHasRoom(kModbusPollMaxPending)) {
    if (modbusReadInputs() == SUCCESS) lastInputPoll = now;
  }

  if (!AktualReadRelay && (forcePoll || ModbusRelayNeedReadback || now - lastRelayPoll >= pollInterval) && modbusQueueHasRoom(kModbusPollMaxPending)) {
    if (modbusReadRelays() == SUCCESS) {
      lastRelayPoll = now;
    }
  }
  if (forcePoll) Rs485ForcePoll = false;
}
...
if (AktualReadInput) {
  ...
  updateWaterLevelDebounce(); // Обновляем антидребезг и инверсную семантику уровнемеров без блокирующих задержек.
  ...
  if (Power_Drain) { // Основная логика активного режима слива.
    ...
    } else if (!DrainCycleRunning && !DrainPauseRunning && DrainPitFullConfirmed) { // Если яма уже заполнена до старта порции.
    ...
    if (DrainCycleRunning) { // Если идет активная порция слива.
      ...
      if ((now - DrainCycleStartedAt >= drainWorkMs) || DrainPitFullConfirmed) { // Останавливаемся по времени или по заполнению ямы.
  ...
  if (!Power_Drain) { // Логика долива работает только когда слив отключен.
    if (Activation_Water_Level) { // Если включен автоматический контроль уровня.
      if (PoolUpperLevelReachedConfirmed) { // Если верхний уровень подтвержден 3 замерами через 1 секунду.
  ...
  queueRelayWriteIfNeeded(2, Rs485ManualRelayState[2], relayWritesThisPass); // Реле 3: свободное ручное реле RS485.
  queueRelayWriteIfNeeded(3, Rs485ManualRelayState[3], relayWritesThisPass); // Реле 4: свободное ручное реле RS485.
  queueRelayWriteIfNeeded(7, Rs485ManualRelayState[7], relayWritesThisPass); // Реле 8: свободное ручное реле RS485.
