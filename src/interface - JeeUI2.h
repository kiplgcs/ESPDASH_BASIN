...
inline void interface(){ // Декларатиынве функции интерфейса
    ...
    UI_POPUP_END();
    UI_DISPLAY("CleanInfoText", CleanInfoText, "ℹ️ Входы, реле RS485 и логика промывки");
...
UI_COLOR("LEDColor", LEDColor, "🎨 Цвет подсветки");
    ...
    UI_NUMBER("DrainPumpMinutes", DrainPumpMinutes, "⏱️ Слив, минут", false);
    UI_NUMBER("DrainPauseMinutes", DrainPauseMinutes, "⏸️ Пауза между сливами, минут", false);
    UI_NUMBER("DrainCycleCount", DrainCycleCount, "🔁 Количество сливов", false);
    UI_DISPLAY_INT("DrainCycleDone", DrainCycleDone, "🔢 Выполнено сливов");
    UI_DISPLAY("WaterLevelStatus", WaterLevelStatus, "ℹ️ Состояние контроля уровня");
    UI_DISPLAY("LevelInfoText", LevelInfoText, "ℹ️ Входы, реле RS485 и логика уровня");
    ...
    "lineWidth:1;pointRadius:3;smooth:false", DS1);
        ...
        UI_POPUP_END();
    UI_DISPLAY("TempInfoText", TempInfoText, "ℹ️ Входы, реле RS485 и логика температуры");
   ...
   static const std::initializer_list<UIOption> dosingOptions{  {"1", "15 сек"},
    ...
    //var("PH_CAL", "PH"+ String(PH, 2) + " : " + String(analogValuePH_Comp)+"mV");
                ...
                UI_NUMBER("PH1_CAL", PH1_CAL, "📉 мВ нижнего pH", false);
                UI_NUMBER("PH2_CAL", PH2_CAL, "📈 мВ верхнего pH", false);
                ...
                UI_BUTTON("Power_H2O2_Button", ManualPulse_H2O2_Request, "gray", "🧪 Проверка работы перельстатического насоса подачи кислоты (вручную)");
        UI_POPUP_END();
    UI_DISPLAY("PhInfoText", PhInfoText, "ℹ️ Входы, реле RS485 и логика pH");
    ...
    "lineWidth:1;pointRadius:3;smooth:false", ppmCl);
        ...
        UI_POPUP_BEGIN("CL", "Калибровка датчика хлора", "Открыть окно калибровки датчика CL хлора}");
            ...
            UI_BUTTON("Power_ACO_Button", ManualPulse_ACO_Request, "gray", "🧴 Проверка работы перельстатического насоса подачи хлора (вручную)");
        UI_POPUP_END();
    UI_DISPLAY("ClInfoText", ClInfoText, "ℹ️ Входы, реле RS485 и логика CL");
...
UI_BUTTON("Pow_Ul_light", Pow_Ul_light, "gray", "🚏 Включить/Отключить освещение вручную");
    ...
    //Реле RS485
    UI_PAGE();
    UI_DISPLAY("Relay485InfoText", Relay485InfoText, "ℹ️ Карта входов и реле RS485");
    UI_DISPLAY("InfoStringDIN_RS485", InfoStringDIN, "📟 Фактическое состояние входов/реле");
