...
inline void interface(){ // Декларатиынве функции интерфейса
    ...
    UI_SELECT_DAYS("DaysSelect", DaysSelect, "📅 Дни промывки");
        ...
        UI_POPUP_BEGIN("Timer_Clean", "🧪 Настройка тамеров последовательности промывки", "🪟 Открыть окно настроики последовательности и таймеров промывки");

            //Промывка запускается всегда по расписанию (день недели + время), независимо от того, что сейчас делает система.
            //Промывка запускается всегда по расписанию (день недели + время), независимо от того, что сейчас делает система.
            //При старте промывки система принудительно переводится в цикл промывки.
            ...
            UI_BUTTON("AirPump", AirPump, "gray", "🔘 Включение компрессора воздуха -Тестовое");
            //T_air_pump — время накачки воздуха компрессором перед переключением клапанов
            ...
            //T_valve_switch — задержка на фактическое переключение трёхходовых клапанов после подачи команды
            ...
            //T_backwash — время обратной промывки: от 15 сек до 5 минут.
            UI_RANGE("T_backwash", TimerBackwashSetting, 0, 300, 1, "⚙️ Время обратной промывки");
            //Шаг 5. Обратная промывка закончена - отключение насоса
            // Пауза на "⚙️ Время на переключение трехходовых клапанов" - для исключения гидроудара
            //Шаг 6. Возврат клапанов в FILTRATION и выключение компрессора
            // Пауза "⚙️ Время на переключение трехходовых клапанов"
            //Шаг 7. Включение насоса
            //Шаг 8. Сброс песка в дренаж - включить соленоид сброса песка после промывки - - включаем реле по RS-485 (выбрать свободное реле от №9 до №16)
            UI_BUTTON("SolSandDump", SolSandDump, "gray", "🔘 Включение соленоида сброса песка после промывки -Тестовое");
            //Шаг 9.  Отключить соленоид сброса песка после промывки
            //Шаг 10.  Завершение промывки и возврат к расписанию. Если до промывки по таймерам был включен режим фильтрации - то продолжается фильтрация иначе насос остановливается.
        UI_POPUP_END();
