...
void onDs18Sensor0Select(const int &value) { // Обрабатываем назначение датчика на температуру бассейна.
  if (assignDs18SensorFromIndex(sensor0, value, Ds18Sensor0Address, kDs18Sensor0StorageKey, InfoString)) { // Пишем выбранный адрес в sensor0 и обновляем статус.
...
void onDs18Sensor1Select(const int &value) { // Обрабатываем назначение датчика на температуру после нагревателя.
  if (assignDs18SensorFromIndex(sensor1, value, Ds18Sensor1Address, kDs18Sensor1StorageKey, InfoString)) { // Пишем выбранный адрес в sensor1 и обновляем статус.
...
void loop() {
  ...
  if (Ds18ScanButton != lastDs18ScanButton) { // Проверяем изменение состояния кнопки поиска.
    ...
    if (Ds18ScanButton == 1) { // Выполняем поиск строго по нажатию.
      InfoString = scanDs18Sensors(); // Сканируем шину и выводим найденные адреса.
