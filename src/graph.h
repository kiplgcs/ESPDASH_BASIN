...
#pragma once
// Заголовочный файл логики хранения, обновления и сериализации графиков

#include <Arduino.h>          // Базовые типы Arduino и millis()
#include <ArduinoJson.h>      // Работа с JSON-конфигурацией графиков
#include <functional>         // std::function для колбэков источников данных
#include <map>                // Хранилища источников и серий графиков
#include <vector>             // Контейнер точек графиков

#include "fs_utils.h"         // Работа с файловой системой (SPIFFS)

using std::vector;

// Структура одной точки графика: временная метка + значение
struct GraphPoint {
  String time;
  float value;
};

// Приводит имя серии к безопасному идентификатору файла
inline String sanitizeSeriesId(const String &series){
  String safe = series;
  safe.replace("/", "_");          // Защита от вложенных путей
  safe.replace("\\", "_");         // Защита от Windows-разделителей
  safe.replace("..", "_");         // Защита от directory traversal
  if(!safe.length()) safe = "main";
  if(safe.length() > 16) safe = safe.substring(0, 16); // Ограничение длины имени файла
  return safe;
}

// Ограничивает количество точек в серии, удаляя самые старые
inline void trimGraphPoints(vector<GraphPoint> &points, size_t limit){
  if(limit == 0) return;
  while(points.size() > limit) points.erase(points.begin());
}

// Минимальный допустимый интервал обновления графика (мс)
inline const unsigned long minGraphUpdateInterval = 100;

// Минимально и максимально допустимое количество точек
inline const int minGraphPoints = 1;
inline const int maxGraphPoints = 50;

// Формирует путь к файлу данных графика
inline String graphDataPath(const String &series){
  return "/graph_" + sanitizeSeriesId(series) + ".dat";
}

// Конфигурация графика: интервал обновления и лимит точек
struct GraphSettings {
  unsigned long updateInterval;
  int maxPoints;
};

// Формирует путь к JSON-файлу конфигурации графика
inline String graphConfigPath(const String &series){
  return "/graph_config_" + sanitizeSeriesId(series) + ".json";
}

// Загружает настройки графика из SPIFFS
...
// Сохраняет точки графика в файл
inline void saveGraphSeries(const String &series, const vector<GraphPoint> &points){
  ...
  for(const auto &p : points){
    f.printf("%s,%.3f\n", p.time.c_str(), p.value); // CSV-формат
  }
  f.close();
}

// Загружает точки графика из файла
...
inline void addGraphPoint(String t, float v){
  ...
  trimGraphPoints(graphPoints, cfg.maxPoints);
  saveGraphSeries("main", graphPoints);
}

// Регистрирует источник данных для графика
inline void registerGraphSource(
  const String &name,
  const std::function<float()> &getter,
  const String &fallback="",
  unsigned long defaultInterval=updateInterval,
  int defaultMaxPoints=maxPoints
){
...
inline void addSeriesPoint(const String &series, const String &t, float value){
  GraphSettings cfg = seriesConfig.count(series)
                        ? seriesConfig[series]
                        : GraphSettings{updateInterval, maxPoints};
