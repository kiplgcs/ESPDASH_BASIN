...
inline const unsigned long maxGraphUpdateInterval = 12UL * 60UL * 60UL * 1000UL; // Максимальный интервал обновления графика: 12 часов.
...
inline void loadGraph(){ // инициализация основной серии графика
  GraphSettings mainCfg{defaultGraphUpdateInterval, 30}; // По умолчанию график обновляется раз в 60 минут.
...
inline void addGraphPoint(String t, float v){ // добавление новой точки в основную серию
  ...
  if(cfg.updateInterval > maxGraphUpdateInterval) cfg.updateInterval = maxGraphUpdateInterval; // Ограничиваем редкие интервалы 12 часами.
...
inline void registerGraphSource( // регистрация новой серии графика
  ...
  if(safeDefaultInterval > maxGraphUpdateInterval) safeDefaultInterval = maxGraphUpdateInterval; // Защита от интервалов больше 12 часов.
