...
String WaterLevelStatus = "Level control OFF";
int DrainPumpMinutes = 20;
int DrainPauseMinutes = 20;
int DrainCycleCount = 5;
int DrainCycleDone = 0;

String CleanInfoText =
  "Inputs: weekdays are set from Web/Nextion; no DIN sensors are used.\n"
  "RS485: relay 3 filter pump, relay 10 air compressor, relay 11 FILTRATION valves, relay 12 BACKWASH valves, relay 13 sand dump.\n"
  "Logic: CleanTimer1 + selected weekdays starts backwash, stops pump, switches valves, washes, then returns filtration.";
String LevelInfoText =
  "Inputs: DIN1 lower pool level, DIN2 upper pool level, DIN3 drain pit level.\n"
  "RS485: relay 14 top-up solenoid, relay 3 filter/drain pump.\n"
  "Logic: lower level opens top-up until upper level, max 1 hour per control cycle; drain disables level control and runs by portions.";
String TempInfoText =
  "Sensors: DS18B20 #1 pool water, DS18B20 #2 after heater.\n"
  "RS485: relay 5 heater, relay 3 filter pump.\n"
  "Logic: heating is allowed only when temperature control is active and filter pump mode is ON.";
String PhInfoText =
  "Sensor: pH ADC input, two-point pH/mV calibration.\n"
  "RS485: relay 7 pH/ACO peristaltic pump, relay 3 filter pump.\n"
  "Logic: pH dosing is allowed only during filtration and when pH is above the set limit.";
String ClInfoText =
  "Sensor: ORP/CL input, control by lower ORP limit.\n"
  "RS485: relay 6 NaOCl/CL peristaltic pump, relay 3 filter pump.\n"
  "Logic: CL dosing is allowed only during filtration, low ORP and normal pH.";
String Relay485InfoText =
  "RS485 relays:\n"
  "1 pool lamp; 2 RGB light; 3 filter/drain pump; 5 heater; 6 CL/NaOCl pump; 7 pH/ACO pump;\n"
  "10 air compressor; 11 FILTRATION valves; 12 BACKWASH valves; 13 sand dump; 14 water top-up; 15 warm floor; 16 outdoor light.\n"
  "RS485 inputs: 1 lower pool level; 2 upper pool level; 3 drain pit level.";
...
bool ManualPulse_H2O2_Request = false;
bool ManualPulse_ACO_Request = false;
...
// ===== Настройки пределов pH/CL =====
float PH_setting = 7.3; // Legacy/HA: верхний предел для включения дозирования pH
float PH_Lower = 7.1;   // Нижний предел гистерезиса pH
float PH_Upper = 7.3;   // Верхний предел гистерезиса pH
float CL_Lower = 0.8;   // Нижний предел гистерезиса свободного хлора, мг/л
float CL_Upper = 1.2;   // Верхний предел гистерезиса свободного хлора, мг/л
...
// Настройка веб-сервера
  void setupServer(){
    ...
    registerUiValueProvider("Power_ACO_Button", [](){ // кнопка ручного импульса светится только во время ручной проверки
        return (ManualPulse_ACO_Request || ManualPulse_ACO_Active) ? String("1") : String("0");
    });
    registerUiValueProvider("Power_H2O2", [](){ // статус дозатора H2O2
      const bool active = Power_H2O2 || ManualPulse_H2O2_Active;
        return active ? String("Работа") : String("Откл.");
    });
    registerUiValueProvider("Power_H2O2_Button", [](){ // кнопка ручного импульса светится только во время ручной проверки
      return (ManualPulse_H2O2_Request || ManualPulse_H2O2_Active) ? String("1") : String("0");
    });
...
".select-days{--day-accent:#4cc3ff;display:flex;flex-wrap:nowrap;gap:8px;" // Контейнер выбора дней недели
 ...
 ".graph-card{position:relative;}"  // Карточка графика
      ...
      ".card:has(#CleanInfoText) div,.card:has(#LevelInfoText) div,.card:has(#TempInfoText) div,.card:has(#PhInfoText) div,.card:has(#ClInfoText) div,.card:has(#Relay485InfoText) div,.card:has(#InfoStringDIN_RS485) div{white-space:pre-line;line-height:1.55;color:#dbeafe;font-size:0.96em;} "
      ...
      "#popup-Cal_PH .popup-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;} "
      "#popup-Cal_PH .card:has(#analogValuePH),#popup-Cal_PH .card:has(#Temper_Reference),#popup-Cal_PH .card:has(#Temper_PH),#popup-Cal_PH .card:has(#Power_H2O2_Button){grid-column:1/-1;} "
      "@media (max-width:620px){#popup-Cal_PH .popup-grid{grid-template-columns:1fr;}} "
