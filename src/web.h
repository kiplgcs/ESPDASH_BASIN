...
inline String PoolWaterLevelLogicInfo = "Логика контроля уровня в бассейне: DI1 нижний уровнемер и DI2 верхний уровнемер работают как замыкающие контакты при падении уровня, поэтому 1 на входе означает, что вода ниже соответствующего датчика. Старт долива разрешается только после 10 последовательных подтверждений нижнего уровня с интервалом 1 секунда. Долив открывает реле 14 на заданное время или до верхнего уровня. Верхний уровень считается достигнутым после 3 последовательных подтверждений с интервалом 1 секунда, после этого реле 14 закрывается.";
inline String DrainPitLogicInfo = "Логика контроля слива бассейна в яму: слив вынесен на отдельную вкладку и не работает одновременно с автоматическим доливом. В режиме слива насос фильтрации на реле 9 работает порциями: заданное время работы, затем пауза, затем следующая порция до заданного количества циклов. DI3 контролирует верхний уровень сливной ямы: так как контакт уровнемера замкнут при падении уровня, заполнение ямы определяется обратным состоянием входа. При заполнении ямы текущая порция слива останавливается и система ждет паузу.";
          ...
          for(auto &e : self->elements){ // Перебор всех элементов вкладки
              ...
              html += "</div>"; // Закрытие карточки графика
                  ...
                  if(maxUpdatePeriod >= 1000){
                      ...
                      if(selected) graphUpdateOptionAdded = true; // 1 секунда уже покрыла текущий интервал.
                      html += String("<option value='") + val + "'" + (selected ? " selected" : "") + ">" + formatIntervalLabel(oneSecond) + "</option>"; // Опция 1 секунда
                  }
                  ...
                  for(unsigned long opt = updateStep; opt <= maxUpdatePeriod; opt += updateStep){
                      ...
                      html += String("<option value='") + val + "'" + (selected ? " selected" : "") + ">" + formatIntervalLabel(opt) + "</option>"; // Опции интервалов
                                          if(maxUpdatePeriod - opt < updateStep) break;
                  }
                  if(!graphUpdateOptionAdded){ // Если период дозирования нестандартный для списка, добавляем его явно.
...
function drawCustomGraph(canvas,data){
  ...
  const eventName = graphEventName(canvas);
  ctx.save();
