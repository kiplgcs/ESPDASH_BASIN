...
inline String WaterLevelInfo = "Контроль уровня: вход 1 нижний уровень бассейна, вход 2 верхний уровень бассейна, вход 3 верхний уровень сливной ямы. Долив: реле 14 открывает клапан на заданное время или до верхнего уровня, затем ждет паузу. Слив: насос фильтрации, реле 9, работает порциями; при срабатывании входа 3 слив сразу останавливается и ждет паузу.";
...
inline bool applyChemistrySettingRequest(const String &id, const String &rawValue) { // Безопасно применяем пределы pH/CL из Web.
  ...
  persistChemicalLimits(); // Сразу сохраняем нормализованные пределы в NVS.
...
inline bool applyWaterControlRequest(const String &id, bool requestedState) { // Централизованно применяем опасные команды уровня воды.
  ...
  if (id == "Power_Topping") { // Обрабатываем ручной соленоидный клапан долива.
    Power_Topping = requestedState && !Power_Drain && !WaterLevelSensorUpper; // Клапан нельзя открыть при сливе или верхнем уровне.
    persistWaterControlModes(); // Сохраняем итоговое безопасное состояние в NVS.
    return true; // Сообщаем вызывающему коду, что команда обработана.
  ...
  saveDebounceTimers[key] = setTimeout(()=>{
    fetch('/save?key='+encodeURIComponent(key)+'&val='+encodeURIComponent(value))
      .finally(()=>{
        ...
        if(el && document.activeElement !== el) clearManualFlag(el, 1500);
      });
...
document.querySelectorAll('input[type=text],input[type=number],input[type=time],input[type=color],select').forEach(el=>{
  ...
  const chemicalInputIds = new Set(['PH_Lower','PH_Upper','CL_Lower','CL_Upper','PH_setting']);
