...
inline String buildPoolWaterLevelStageInfo() { // Текущий этап контроля уровня бассейна для Web-страницы.
  ...
  if (PoolLowerLevelLowConfirmed) return "Контроль уровня: нижний уровень подтвержден, ожидание запуска долива."; // Нижний уровень подтвержден, автоматика запустит долив на следующем проходе.
       ...
       html += "</div>"; // Закрытие карточки изображения
          ...
          for(auto &e : self->elements){ // Перебор всех элементов вкладки
              ...
              if(e.type=="displayGraph" || e.type=="displayGraphJS"){ // Графики
                  ...
                  const size_t maxUpdateOptions = 160; // Максимум вариантов, чтобы шаг 5 минут доходил до 12 часов.
              ...
              html += "</div>"; // Закрытие карточки графика
                  ...
                  addGraphIntervalOption(minGraphUpdateInterval); // Минимальный ручной интервал: 5 секунд.

                  for(unsigned long opt = updateStep; opt <= maxUpdatePeriod; opt += updateStep){
                      if(opt != minGraphUpdateInterval) addGraphIntervalOption(opt); // Опции от шага до 12 часов.
