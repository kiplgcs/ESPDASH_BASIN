...
inline int8_t ZB_UART_RX_PIN   = -1; // RX ESP32  ← TX CC2652
inline int8_t ZB_UART_TX_PIN   = -1; // TX ESP32  → RX CC2652
inline uint32_t ZB_UART_BAUD   = 115200; // Скорость UART Zigbee
// Управляющие пины Zigbee-модуля // Конфигурация управляющих линий
inline int8_t ZB_RESET_PIN     = -1; // RESET CC2652 (активный уровень зависит от модуля)
inline int8_t ZB_BOOT_PIN      = -1; // BOOT / BSL (опционально, -1 если не используется)
