...
public:
    ...
    void sidebarEspStatus(const String &id, const String &label){ registerSidebarBlock("espStatus", id, label); } // декларативный индикатор связи с ESP32
    void sidebarThemeColor(const String &id, const String &label){ registerSidebarBlock("themeColor", id, label); } // декларативная карточка цвета темы
    void systemPage(const String &id, const String &title, const String &type){ registerSystemPage(id, title, type); } // декларативная служебная страница
    void wifiSettingsPage(const String &id="wifi", const String &title="WiFi Settings"){ systemPage(id, title, "wifi"); } // страница WiFi
    void statisticsPage(const String &id="stats", const String &title="Statistics"){ systemPage(id, title, "stats"); } // страница статистики
  ...
  void image(const String &id, const String &filename, const String &style=""){ // добавляет изображение с поддержкой позиционирования и inline-стилей
    ...
    void rs485Panel(const String &id, const String &imagePath, const String &label){ // добавляет декларативную панель управления платой RS485
        addElement("rs485Panel", id, label, imagePath); // регистрирует специальный элемент RS485 в текущей вкладке
    }
...
private:
    ...
    void resetDash(){ // полностью очищает текущее описание UI
        ...
        resetSystemUiDeclarations(); // очищает декларативные служебные блоки Web UI
...
class UIRs485PanelElement : public UIDeclarativeElement { // Декларативная панель платы RS485 16CH + DI16
public:
    UIRs485PanelElement(const String &elementId, const String &imagePath, const String &elementLabel)
        : UIDeclarativeElement(elementId, elementLabel), image(imagePath) {}

    void build(OABuilder &builder) override{ builder.rs485Panel(id, image, label); }
    void load() override{ loadRs485PanelSettings(); }
    void save() const override{}
...
inline void onAcoDosingPeriodChange(const int &value){ // Обработчик выбора периода дозирования кислоты ACO из Web.
    ACO_Work = sanitizeDosingPeriodValue(value); // Приводим выбранный код к разрешенному списку.
    holdNextionDispensersReads(); // После изменения из Web временно не читаем старое значение с Nextion.
    saveValue<int>("ACO_Work", ACO_Work); // Сразу сохраняем исправленное значение в NVS.
}

inline void onH2O2DosingPeriodChange(const int &value){ // Обработчик выбора периода дозирования NaOCl из Web.
    H2O2_Work = sanitizeDosingPeriodValue(value); // Приводим выбранный код к разрешенному списку.
    holdNextionDispensersReads(); // После изменения из Web временно не читаем старое значение с Nextion.
    saveValue<int>("H2O2_Work", H2O2_Work); // Сразу сохраняем исправленное значение в NVS.
}
...
#define UI_SIDEBAR_ESP_STATUS(id, label) oab.sidebarEspStatus(id, label) // индикатор связи ESP32 в левой панели
#define UI_SIDEBAR_THEME_COLOR(id, label) oab.sidebarThemeColor(id, label) // выбор цвета темы в левой панели
#define UI_WIFI_SETTINGS_PAGE() oab.wifiSettingsPage() // служебная страница WiFi Settings
#define UI_STATISTICS_PAGE() oab.statisticsPage() // служебная страница Statistics
#define UI_PROFILE_PAGE() oab.profilePage() // служебная страница профиля
#define UI_MQTT_SETTINGS_PAGE() oab.mqttSettingsPage() // служебная страница MQTT
...
// декларативная панель управления платой RS485 16CH + DI16
#define UI_RS485_PANEL(id, imagePath, label) \
    do { static UIRs485PanelElement UI_UNIQUE_NAME(ui_rs485_panel_)(id, imagePath, label); UI_REGISTER_ELEMENT(UI_UNIQUE_NAME(ui_rs485_panel_)); } while(false)
