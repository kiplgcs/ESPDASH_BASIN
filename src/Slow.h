...
if(Nx_dim_id < 20 /*&& !Act_PH && !Act_Cl*/) {period_slow_Time = 1000;//} // Приглушенный экран все равно опрашиваем раз в секунду для синхронизации настроек.
...
if(Nx_page_id == 0){
  ...
  myNex.writeStr("page0.t0.txt", formatTemperatureString(DS1, DS1Available)); // Показываем температуру воды в бассейне на главной странице.
  myNex.writeStr("page0.t1.txt", formatTemperatureString(DS2, DS2Available)); // Показываем температуру после нагревателя на главной странице.
  myNex.writeStr("page0.t2.txt", formatTemperatureString(DS1, DS1Available)); // Дублируем доступную температуру в третье поле, если оно есть в HMI.
...
myNex.writeStr("Service.t0.txt", serviceNextionText()); // На странице сервиса показываем Wi-Fi/IP/RSSI/heap понятным текстом.
...
if(Nx_page_id == 9){ // На открытой странице дозаторов обновляем только измерения, а настройки пишет Nextion_Transmit по изменению.
  myNex.writeStr("Dispensers.t0.txt", "PH: " + String(PH)); // Показываем текущий pH без перезаписи полей настройки.
  myNex.writeStr("Dispensers.t1.txt", "Cl: " + String(ppmCl)); // Показываем текущий хлор без перезаписи полей настройки.
}

#if 0 // Старый блок перезаписывал настройки дозаторов каждую секунду и оставлен выключенным для безопасности.
  ...
  myNex.writeNum("Dispensers.cb0.val", nextionDosingComboIndexFromMode(ACO_Work)); // Как часто включается перельстатический насос

  //myNex.writeNum("Dispensers.sw3.val", Activation_Timer_H2O2? 1 : 0);//Работа перельстатического насоса
  myNex.writeNum("Dispensers.cb1.val", nextionDosingComboIndexFromMode(H2O2_Work)); // Как часто включается перельстатический насос
...
switch (flag_slow) {
  ...
  uint32_t lumenRawSum = 0; // Усредняем несколько быстрых замеров ADC, чтобы не показывать случайный всплеск.
  for (uint8_t lumenSample = 0; lumenSample < 8; ++lumenSample) { // 8 замеров достаточно для диагностики и не тормозит loop.
    lumenRawSum += analogRead(3); // Читаем ADC GPIO3 без delay, чтобы синхронизация Nextion/Web не зависала.
  }
  Lumen_Ul_Raw = static_cast<int>(lumenRawSum / 8UL); // Сохраняем сырой АЦП, чтобы видеть реальный сигнал датчика при диагностике.
  ...
  if (Lumen_Ul_Raw > 4000) InfoString2 += " - GPIO3 почти 3.3V"; // Если в темноте видно это сообщение, вход аппаратно удерживается высоким уровнем.
...
const bool filtrRelayState = Power_Filtr || ReadRelayArray[8];
...
switch (sync_step) {
  ...
  case 7:
    ...
    if (!onDispensersPage) { // Не перезаписываем настройки дозаторов
      myNex.writeNum("Dispensers.cb0.val", nextionDosingComboIndexFromMode(ACO_Work)); // В Nextion пишем индекс строки ComboBox.
    }
    sync_step++;
    break;
  case 8:
    if (!onDispensersPage) { // Не перезаписываем настройки дозаторов
      myNex.writeNum("Dispensers.cb1.val", nextionDosingComboIndexFromMode(H2O2_Work)); // В Nextion пишем индекс строки ComboBox.
