...
inline String nextionShortFloat(float value, uint8_t decimals = 2) {
  String text = String(value, static_cast<unsigned int>(decimals));
  if (text.length() > 5) text = text.substring(0, 5);
  return text;
}

inline String wifiStatusLabel(wl_status_t status) {
  switch (status) {
...
if(Nx_page_id == 0){
  ...
  myNex.writeStr("page0.b7.txt", nextionShortFloat(ppmCl)); //Содержание хлора
  myNex.writeStr("page0.b8.txt", nextionShortFloat(PH)); //Кислотность воды

  myNex.writeStr("page0.t0.txt", formatTemperatureString(DS1, DS1Available)); // Температура в бассейне
  myNex.writeStr("page0.t1.txt", formatTemperatureString(DS2, DS2Available)); // Температура после нагревателя - подача нагретой воды в бассейн
  myNex.writeStr("page0.t2.txt", "N/A"); // Температура теплоносителя в нагреватель: датчик в проекте пока не назначен
...
if(Nx_page_id == 5){
  ...
  myNex.writeNum("heat.h0.maxval", 35);
...
const bool wifiApActive = (currentWifiMode == WIFI_AP || currentWifiMode == WIFI_AP_STA);
String serviceInfo =
  "WiFi: " + wifiStatusLabel(WiFi.status()) + "\r\n" +
  "SSID: " + (WiFi.SSID().length() ? WiFi.SSID() : String("-")) + "\r\n" +
  "RSSI: " + (WiFi.status() == WL_CONNECTED ? String(WiFi.RSSI()) + " dBm" : String("-")) + "\r\n" +
  "STA IP: " + (WiFi.status() == WL_CONNECTED ? WiFi.localIP().toString() : String("-")) + "\r\n" +
  "AP IP: " + (wifiApActive ? WiFi.softAPIP().toString() : String("-")) + "\r\n" +
  "MAC: " + WiFi.macAddress() + "\r\n" +
  "Uptime: " + String(millis() / 1000UL) + " s\r\n" +
  "Heap: " + String(ESP.getFreeHeap()) + "\r\n" +
  "Nextion page: " + String(Nx_page_id) + "\r\n" +
  "Nextion dim idle: " + String(Nx_dim_id) + "%, sync " + String(period_slow_Time / 1000) + "s";
static bool serviceInfoWritten = false;
static unsigned long lastServiceInfoMs = 0;
if (!serviceInfoWritten || millis() - lastServiceInfoMs >= 30000UL) {
  myNex.writeStr("Service.t0.txt", serviceInfo);
  lastServiceInfoMs = millis();
...
// Do not push data into inactive Nextion pages. The current page is updated above.
return;
...
switch (sync_step) {
  ...
  case 4:
    ...
    break;
  case 5:
    myNex.writeStr("page0.b7.txt", nextionShortFloat(ppmCl));
    myNex.writeStr("page0.b8.txt", nextionShortFloat(PH));
    myNex.writeStr("page0.t0.txt", formatTemperatureString(DS1, DS1Available));
    myNex.writeStr("page0.t1.txt", formatTemperatureString(DS2, DS2Available));
    myNex.writeStr("page0.t2.txt", "N/A");
    sync_step++;
    break;
   case 6:
