...
inline int MotorSpeedPauseSetting = 150; // Пауза между прокрутками текста (мс)
inline int MotorSpeedCommentSetting = 10; // Скорость изменения текста (Comment)
...
inline String Comment1; // Строка напоминания
...
inline String Ds18HelpText = "Шаг 1: нажмите поиск. Шаг 2: выберите индекс и назначьте датчики для помещения/улицы. Для отвязки выберите \"❌ Отвязать датчик\"."; // Подсказка по DS18B20.

inline int Ds18ScanButton = 0;   // Кнопка сканирования DS18B20.
inline int Ds18Sensor0Index = -2; // Индекс датчика для sensor0 (-2: не применять при старте, -1: отвязать).
inline int Ds18Sensor1Index = -2; // Индекс датчика для sensor1 (-2: не применять при старте, -1: отвязать).
inline String Ds18Sensor0Address; // Адрес назначенного sensor0.
inline String Ds18Sensor1Address; // Адрес назначенного sensor1.
...
inline String RoomTempHero = "--"; // Крупное значение температуры в помещении для страницы графика
inline String OutdoorTempHero = "--"; // Крупное значение температуры на улице для страницы графика
...
inline bool reminder = false; // Включить напоминание по времени
inline int ReminderTimerON = 0; // Таймер напоминания: начало
inline int ReminderTimerOFF = 0; // Таймер напоминания: конец
...
extern bool DS1Assigned;
extern bool DS2Assigned;
String formatTemperatureString(float value, bool available);
void onDs18Sensor0Select(const int &value); // Callback выбора датчика для sensor0.
void onDs18Sensor1Select(const int &value); // Callback выбора датчика для sensor1.
...
inline constexpr const char *kRtcCorrectionStorageKey = "rtcCorrMsMin"; // Короткий ключ NVS (<=15 символов).

int rtcCorrectionMillisPerMinute = 0; // Ручная коррекция RTC: миллисекунд добавлять/убавлять каждую минуту (работает без NTP)
inline int normalizeRtcCorrectionMsPerMinute(int value){
...
public:
  static String timerStorageKey(const String &id, const String &suffix){
    if(id == "UlLightTimer"){
      ...
      if(id == "ReminderTimer"){
      return String("RemTimer") + suffix; // Укороченный ключ хранения для ReminderTimer
    }
...
// Настройка веб-сервера
  void setupServer(){
    ...
    registerUiValueProvider("RoomTempHero", [](){ return formatTemperatureString(DS1, DS1Available); }); // крупный индикатор температуры помещения
    registerUiValueProvider("OutdoorTempHero", [](){ return formatTemperatureString(DS2, DS2Available); }); // крупный индикатор температуры улицы
       ...
       html += "</div>"; // Закрытие карточки изображения
          ...
          for(auto &e : self->elements){ // Перебор всех элементов вкладки
              ...
              if(e.type=="displayGraph" || e.type=="displayGraphJS"){ // Графики
                  ...
                  String signedTemperatureRaw = readSetting("signedTemperature"); // Проекция температуры с отрицательным диапазоном
                  bool signedTemperature = signedTemperatureRaw == "1" || signedTemperatureRaw == "true" || signedTemperatureRaw == "yes";
              ...
              if(e.type=="clockSelect"){ // Панель настройки времени
                  html += "<div class='card compact stats-card time-settings-card'>"
                          "<div class='stat-group'><div class='stat-heading'>"+e.label+"</div>"
                          ...
                          "<div class='time-settings-field'><label for='gmtOffset'>Часовой пояс</label><select id='gmtOffset'></select></div>"
                            "<div class='time-settings-field'><label for='rtcHourlyCorrection'>Коррекция RTC, мс/мин</label><input id='rtcHourlyCorrection' type='number' min='-1000' max='1000' step='1'></div>"
...
function saveProfileSettings(){
 const statusEl = document.getElementById('profile-status');
  ...
  function applyManualTime(){
    ...
    if(tzSelect && tzSelect.value !== ''){
      payload.append('gmtOffset', tzSelect.value);
          ...
          const rtcCorrectionInput = document.getElementById('rtcHourlyCorrection');
    if(rtcCorrectionInput && rtcCorrectionInput.value !== ''){
      payload.append('rtcHourlyCorrection', rtcCorrectionInput.value);
    }
  ...
  function initTimeSettings(){
    ...
    const tzSelect = document.getElementById('gmtOffset');
    const rtcCorrectionInput = document.getElementById('rtcHourlyCorrection');
    [dateInput, timeInput, tzSelect, rtcCorrectionInput].forEach(el=>{
      if(!el) return;
      listenToManual(el);
    });
  }
...
function createTemperatureGraphProjection(points, height){
  let minValue = 0;
  let maxValue = 0;
  let hasNumericValues = false;

  for(let i=0;i<(points || []).length;i++){
    const numeric = Number(points[i].value);
  ...
  // Всегда включаем ноль в диапазон, чтобы корректно отображались
  // положительный и отрицательный сектора на одном графике.
  minValue = Math.min(minValue, 0);
  maxValue = Math.max(maxValue, 0);

  if(minValue === maxValue){
    const basePadding = Math.max(Math.abs(minValue) * 0.1, 1);
    minValue -= basePadding;
    maxValue += basePadding;
  }

  const valueRange = maxValue - minValue;
  const padding = Math.max(valueRange * 0.12, 0.5);
  const projectedMin = minValue - padding;
  const projectedMax = maxValue + padding;
  const projectedRange = projectedMax - projectedMin;

  const mapY = (rawValue)=>{
    const numeric = Number(rawValue);
    if(!Number.isFinite(numeric) || projectedRange <= 0) return height / 2;
    const ratio = (numeric - projectedMin) / projectedRange;
    const y = height - ratio * height;
    return Math.max(0, Math.min(height, y));
  };

  return {
    minValue: projectedMin,
...
function drawCustomGraph(canvas,data){
  ...
  const pointsToDraw = data.slice(-maxPoints);
    const useSignedTemperature = canvas.dataset.signedTemperature === '1';
  const projection = useSignedTemperature
    ? createTemperatureGraphProjection(pointsToDraw, height)
    : null;
  const defaultMapY = (rawValue)=>{
    const numeric = Number(rawValue);
    if(!isFinite(numeric)) return height / 2;
    const y = height - (numeric / 50.0) * height;
    return Math.max(0, Math.min(height, y));
  };
  const mapY = projection ? projection.mapY : defaultMapY;
  const lineColor = canvas.dataset.lineColor || '#4CAF50';
  const pointColor = canvas.dataset.pointColor || '#ff0000';
  const stepX = pointsToDraw.length > 1
    ? (width / (pointsToDraw.length - 1))
    : 0;

  if(projection && projection.hasNegative && projection.hasPositive){
    ctx.strokeStyle = 'rgba(255,255,255,0.22)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(0, projection.zeroY);
    ctx.lineTo(width, projection.zeroY);
    ctx.stroke();
  }
  ...
  for(let i=0;i<pointsToDraw.length;i++){
    const x = stepX * i;
    const y = mapY(pointsToDraw[i].value);
  ...
  graphDataCache.set(canvas, { points: pointsToDraw, projection, useSignedTemperature });
...
function showGraphTooltip(canvas, evt){
  ...
  const graphState = graphDataCache.get(canvas);
  const points = graphState && graphState.points ? graphState.points : [];
  ...
  const mapY = graphState && graphState.useSignedTemperature && graphState.projection && graphState.projection.mapY
    ? graphState.projection.mapY
    : ((val)=>{
        const numeric = Number(val);
        if(!isFinite(numeric)) return height / 2;
        const yVal = height - (numeric / 50.0) * height;
        return Math.max(0, Math.min(height, yVal));
      });
  let y = mapY(point.value);
  ...
  customGraphCanvases.forEach(canvas=>{
    ...
    fetch('/live').then(r=>r.json()).then(j=>{
        ...
        if(typeof j.gmtOffset !== 'undefined') updateSelectValue('gmtOffset', j.gmtOffset);
            if(typeof j.rtcHourlyCorrection !== 'undefined') updateInputValue('rtcHourlyCorrection', j.rtcHourlyCorrection);
  ...
  fetch('/setjpg?val='+x).then(resp=>{
    ...
    // ---------------- SAVE ----------------
      ...
      //Вывод информации из EEPROM  на WEB страницу
        ...
        else if(key=="rtcHourlyCorrection") {
          int corrMs = valStr.toInt();
          int corr = normalizeRtcCorrectionMsPerMinute(corrMs);
          rtcCorrectionMillisPerMinute = corr;
          saveValue<int>(kRtcCorrectionStorageKey, corr);
        }
    ...
    server.on("/time/set", HTTP_POST, [](AsyncWebServerRequest *r){
      ...
      String rtcCorrectionStr = paramOr("rtcHourlyCorrection");
      if(offsetStr.length()){
        ...
        if(rtcCorrectionStr.length()){
        int corrMs = rtcCorrectionStr.toInt();
        int corr = normalizeRtcCorrectionMsPerMinute(corrMs);
        rtcCorrectionMillisPerMinute = corr;
        saveValue<int>(kRtcCorrectionStorageKey, corr);
      }
    ...
    server.on("/live", HTTP_GET, [](AsyncWebServerRequest *r){
      ...
      doc["rtcHourlyCorrection"] = rtcCorrectionMillisPerMinute; // Коррекция RTC (миллисекунд в минуту)
