...
//if(myNex.currentPageId == 0){ }     //Проверяем находимся ли мы на странице "0" - page0 и только тогда обновляем эти элементы.
...
switch (flag_slow) {
  ...
  Lumen_Ul=analogRead(A0); //pin 36 - освещенность на улице
  //jee.var("Lumen_Ul", String(Lumen_Ul));
   ...
   if(Lumen_Ul != Saved_Lumen_Ul) {Saved_Lumen_Ul = Lumen_Ul;
     ...
     delay(5);
     jee.var("Lumen_Ul", String(Lumen_Ul_percent)); } //освещенность на улице в процентах
...
//   break;
  ...
  // if (AktualReadInput) {
  //     // // Создаем массив указателей на переменные Input1, ..., Input16
  //     // bool* Inputs[16] = { &Input1, &Input2, &Input3, &Input4, &Input5, &Input6, &Input7, &Input8,
  //     //                     &Input9, &Input10, &Input11, &Input12, &Input13, &Input14, &Input15, &Input16 };

  //     // // Используем цикл для присвоения значений из массива ReadInputArray переменным Input1, ..., Input16
  //     // for (int i = 0; i < 16; ++i) {
  //     //     *Inputs[i] = ReadInputArray[i];
  //     // }
  //     String Input8 = "", Input9_16 = "";
  //     jee.var("info", ""); 
  //     for (int i = 0; i < 16; ++i) {
  //     if (i<=7) Input8 += " " + String(i + 1) + "=" + (ReadInputArray[i] ? "1" : "0") + ",";
  //     if (i>7) Input9_16 += " " + String(i + 1) + "=" + (ReadInputArray[i] ? "1" : "0") + ",";
  //     }

      
  //     jee.var("info", "In1: " + Input8 + ":::" + "In2: " + Input9_16);
  ...
  // myNex.writeStr("cir 100,100,30,RED"); 
  ...
  // Играть Аудио - https://youtu.be/AgJts3TwSKo
  

// if(myNex.currentPageId == 0){ //Это работает передаем либо на 0 страницу либо на 5 страницу
//   myNex.writeStr("t0.txt", "ge0!");
// }else if(myNex.currentPageId == 5){
//   myNex.writeStr("t0.txt", "ge1!");
// }
  ...
  // if(Sider_heat>DS1+0.1){ //Передаем на Nextion -пераметр для включения или отключения нагрев воды в бассейне по температуре воды 
  // myNex.writeNum("page0.va0.val", 1);
  // } else if (Sider_heat<DS1-0.1) {
